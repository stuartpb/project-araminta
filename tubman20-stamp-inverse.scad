...
module from_jackson_bottom_left(extents) {
  translate([-jackson_width/2,-jackson_height]) children();
}
...
module stamp_negative () {
  union() {
    translate([0,0,backing_depth]) linear_extrude(backing_depth + fudge) import("jackson-silhouette.svg");
    translate([0, tubman_y_offset, backing_depth]) linear_extrude(backing_depth + fudge) import("tubman-silhouette.svg");
